...
// Convert UTF-16 text to a specific ANSI codepage (what the engine expects).
[[nodiscard]] inline std::string wstring_to_codepage(const std::wstring& s, UINT codepage)
{
    if (s.empty()) {
        return {};
    }
    const int size_needed = WideCharToMultiByte(codepage, 0, s.c_str(),
                                                static_cast<int>(s.size()), nullptr, 0, nullptr, nullptr);
    std::string result(static_cast<size_t>(size_needed), '\0');
    WideCharToMultiByte(codepage, 0, s.c_str(), static_cast<int>(s.size()),
                        result.data(), size_needed, nullptr, nullptr);
    return result;
}
